...
namespace AdventOfCode
{
  public class Puzzle04
  {
    ...
    {
      var guardData = ParseGuardData();

      var guardSleep = new Dictionary<int, Dictionary<int, int>>();
      var guardSleepCount = new Dictionary<int, int>();

      foreach (var key in guardData.Keys)
      {

        guardSleep.Add(key,new Dictionary<int, int>());
        guardSleepCount.Add(key,0);
        for (var i = 0; i <= 59; i++)
        {
          guardSleep[key].Add(i, 0);
        }

        var startOfSleep = 0;
     
        foreach (var action in guardData[key])
        {
          if (action.Item2 == "falls asleep")
          {
            startOfSleep = action.Item1.Minute;
          }
          if (action.Item2 == "wakes up")
          {
            for (var i = startOfSleep; i < action.Item1.Minute; i++)
            {
              guardSleep[key][i] += 1;
              guardSleepCount[key] += 1;
            }
          }
        }
      }
    ...
    private static Dictionary<int, List<Tuple<DateTime, string>>> ParseGuardData()
    {
      var guardData = new Dictionary<int, List<Tuple<DateTime, string>>>();

      var input = ParseInput();
      var currentGuard = -1;
      foreach (var date in input)
      {
        if (date.Item2.StartsWith("Guard"))
        {
          currentGuard = Convert.ToInt32(date.Item2.Split(' ')[1].Replace("#", String.Empty));
        }

        if (!guardData.ContainsKey(currentGuard))
        {
          guardData.Add(currentGuard, new List<Tuple<DateTime, string>>());
        }

        guardData[currentGuard].Add(new Tuple<DateTime, string>(date.Item1, date.Item2));
      }

      return guardData;
    }

    private static List<Tuple<DateTime, string>> ParseInput()
    {
      var result = new List<Tuple<DateTime, string>>();
      foreach (var item in Input().Split(Environment.NewLine))
      {
        var split = item.Split(']');
        result.Add(new Tuple<DateTime, string>(Convert.ToDateTime(split[0].Replace("[", "")), split[1].Trim()));
      }
      return result.OrderBy(o => o.Item1).ToList();
    }

    public static string Input()
    {
      return @"[1518-07-03 23:58] Guard #2437 begins shift
